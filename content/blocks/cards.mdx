export const Cards = ({ ...props }) => (
  <Section py="8|12" {...props}>
    <Container>
      <Grid col="1|2|3" mt="8|10|12" gap="4|6|8">
        <Card
          url="https://reflexjs.org/learn"
          icon="book"
          heading="Kryptografie"
          description="Read the quick-start tutorials."
        />
        <Card
          url="https://reflexjs.org/docs"
          icon="book-open"
          heading="Signieren"
          description="Fortgeschrittene oder qualifizierte Signaturen, Stellvertreter- oder Fernsignaturen."
        />
        <Card
          url="https://reflexjs.org/library/blocks"
          icon="grid"
          heading="Zertifikate"
          description="Browse the blocks library."
        />
        <Card
          url="https://reflexjs.org/learn"
          icon="book"
          heading="Kontakte (Peers)"
          description="Read the quick-start tutorials."
        />
        <Card
          url="https://reflexjs.org/docs"
          icon="book-open"
          heading="Sicherer Datenraum"
          description="Read the Reflex docs."
        />
        <Card
          url="https://reflexjs.org/library/blocks"
          icon="grid"
          heading="Elektronischer Rechtsverkeher"
          description="Browse the blocks library."
        />
      </Grid>
    </Container>
  </Section>
);

export const Card = ({ icon, heading, description, url, ...props }) => (
  <Flexbox
    flexDirection="column"
    textAlign="center"
    borderWidth="1px"
    rounded="lg"
    p="4|6|8"
    position="relative"
    hoverBorderColor="primary"
    {...props}
  >
    <Link
      href={url}
      position="absolute"
      w="full"
      h="full"
      top="0"
      target="_blank"
      rel="noopener"
    >
      <VisuallyHidden>{heading}</VisuallyHidden>
    </Link>
    <Flexbox
      bg="primary"
      color="white"
      rounded="full"
      size="16"
      alignItems="center"
      justifyContent="center"
    >
      <Icon name={icon} size="6" />
    </Flexbox>
    <H3 fontSize="2xl" mt="4|6" mb="0">
      {heading}
    </H3>
    <P mb="0" mt="2">
      {description}
    </P>
  </Flexbox>
);